...
async function createAccount(req, res, next) {
  ...
  try {
    ...
    if (data.length !== 0) {
      return res
        .status(409)
        .send('Email no disponible: email ya pertenece a otro usuario.');
    }
  } catch (e) {
    console.error(e);
...
async function sendWelcomeEmail(accountData) {
  ...
  title = '¡Bienvenid@ a Castiel Traducciones!';
  intro = `${username}, muchas gracias por confiar en nosotros.`;
  description = 'Nuestro servicio web le permite:';
  benefits = `<li style="padding - bottom: 10px">Crear un perfil en el que publicitar sus servicios de traducción.</li>
		<li>Ampliar su red de traductores.</li>`;

  const message = {
    to: accountData.email,
    from: 'castieltraducciones@yopmail.com',
    subject: '¡Bienvenido!',
    html: `<html lang="es">
	<head>
		<meta charset="UTF-8" />
